...
def recommend_hospital3(path, emergency, start_lat, start_lng, c_id, c_key):
    ...
    sorted_temp['지역'] = sorted_temp.apply(lambda row: get_region(row['위도'], row['경도']), axis=1)
    # 결과를 JSON 형태로 변환
...
def get_hospital(text: str,latitude: float,longitude: float):
    path=''
...
async def recommend_emergency_with_record(file: UploadFile = File(...), latitude: float = Query(...), longitude: float = Query(...)):
    ...
    result = None
    if predicted_class <= 3 :
        result = {"status":200, "scale": predicted_class, "text": text, "message": "응급실을 추천합니다.", "keyword": keywords}
